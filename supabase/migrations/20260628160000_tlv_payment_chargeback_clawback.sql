-- TLV Payment Engine TODO-06 P0 — Chargeback / Refund / Clawback
-- Design: reports/tlv-payment-chargeback-clawback-design.md
-- Staging apply only; production manual gate.

-- ---------------------------------------------------------------------------
-- ENUMs
-- ---------------------------------------------------------------------------
do $$ begin
  create type tlv.payment_reversal_kind as enum (
    'refund',
    'dispute_open',
    'dispute_lost',
    'dispute_won'
...
do $$ begin
  create type tlv.payment_reversal_status as enum (
    'pending',
    'applied',
    'failed'
  );
...
-- ---------------------------------------------------------------------------
-- revenue_ledger — allow negative gross/net for adjustment reversals only
-- ---------------------------------------------------------------------------
alter table tlv.revenue_ledger
  drop constraint if exists revenue_ledger_amounts_nonneg_chk;

alter table tlv.revenue_ledger
  drop constraint if exists revenue_ledger_amounts_chk;

alter table tlv.revenue_ledger
  add constraint revenue_ledger_amounts_chk check (
    (
      event_kind <> 'adjustment'
      and gross_amount_jpy >= 0
      and fee_amount_jpy >= 0
      and net_amount_jpy >= 0
      and infra_cost_jpy >= 0
    ...
    or (
      ...
      and infra_cost_jpy >= 0
      and creator_payout_jpy >= 0
      and net_amount_jpy = gross_amount_jpy - fee_amount_jpy
      and platform_revenue_jpy = net_amount_jpy - infra_cost_jpy - creator_payout_jpy
    )
  );

comment on constraint revenue_ledger_amounts_chk on tlv.revenue_ledger is
  'Non-adjustment rows non-negative; adjustment allows negative gross/net for chargeback/refund reversals';

-- ---------------------------------------------------------------------------
-- payments — charge lookup index
-- ---------------------------------------------------------------------------
create index if not exists payments_stripe_charge_idx
  on tlv.payments (stripe_charge_id)
  where stripe_charge_id is not null;

-- ---------------------------------------------------------------------------
-- payment_reversals — refund/dispute audit trail
-- ---------------------------------------------------------------------------
create table if not exists tlv.payment_reversals (
  id                  uuid primary key default gen_random_uuid(),
  payment_id          uuid not null references tlv.payments (id) on delete restrict,
  provider_event_id   uuid references tlv.payment_provider_events (id) on delete set null,
  reversal_kind       tlv.payment_reversal_kind not null,
  status              tlv.payment_reversal_status not null default 'pending',
  amount_jpy          bigint not null default 0,
  coins               integer not null default 0,
  coins_shortfall     integer not null default 0,
  reason              text,
  stripe_dispute_id   text,
  metadata            jsonb not null default '{}'::jsonb,
  created_at          timestamptz not null default now(),
  constraint payment_reversals_amount_jpy_chk check (amount_jpy >= 0),
  constraint payment_reversals_coins_chk check (coins >= 0),
  constraint payment_reversals_shortfall_chk check (coins_shortfall >= 0),
  constraint payment_reversals_reason_len_chk check (
    reason is null or char_length(reason) between 1 and 256
  )
);

comment on table tlv.payment_reversals is
  'Refund/dispute reversal audit — partial refund accumulation · shortfall · Ops FinOps flags';
comment on column tlv.payment_reversals.provider_event_id is
  'FK to payment_provider_events.id — Stripe event.id dedup via provider_events UNIQUE';
comment on column tlv.payment_reversals.coins_shortfall is
  'Coins that could not be clawed (wallet insufficient) — wallet frozen · Ops review';

create unique index if not exists payment_reversals_provider_event_uniq
  on tlv.payment_reversals (provider_event_id)
  where provider_event_id is not null;

create unique index if not exists payment_reversals_dispute_phase_uniq
  on tlv.payment_reversals (payment_id, reversal_kind, stripe_dispute_id)
  where stripe_dispute_id is not null
    and reversal_kind in ('dispute_open', 'dispute_lost', 'dispute_won');

create index if not exists payment_reversals_payment_idx
  on tlv.payment_reversals (payment_id, created_at desc);

create index if not exists payment_reversals_status_idx
  on tlv.payment_reversals (status, created_at desc);
...
-- ---------------------------------------------------------------------------
-- RLS — payment_reversals (admin only · TODO-07 pattern)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Privileges — service_role only for new RPCs
-- ---------------------------------------------------------------------------
