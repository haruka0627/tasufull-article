-- TLV スキーマ基盤: create schema tlv + 19テーブル + ENUM + trigger + seed
-- 正本: db/tlv_schema.sql v1.2.5
-- 前提: なし（新規スキーマ）
-- 位置: 20260627100000_match_beta_allowlist.sql の後
--       20260628100000_live_p0_schema.sql の前
--       20260628120000_tlv_payment_phase2_rpc.sql（tlv.* 依存5件）の前提
-- 冪等: 全 DDL が既存環境で再実行可能
--        create schema / table / index は if not exists
--        create type は DO ブロックで存在チェック
--        create trigger は drop if exists + create
--        alter table は add column if not exists / drop constraint if exists
...
-- =========================================================================
-- ENUM types (14 types — idempotent via DO block)
-- =========================================================================

do $$
begin
  if not exists (select 1 from pg_type where typname = 'rank_tier' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.rank_tier as enum ('bronze', 'silver', 'gold', 'platinum', 'diamond', 'legend');
  end if;
  if not exists (select 1 from pg_type where typname = 'override_tier' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.override_tier as enum ('none', 'tier_90', 'tier_95');
  end if;
  if not exists (select 1 from pg_type where typname = 'score_axis' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.score_axis as enum ('FS', 'ES', 'GS', 'TS');
  end if;
  if not exists (select 1 from pg_type where typname = 'payment_channel' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.payment_channel as enum ('web_stripe', 'ios_iap', 'android_iap', 'adsense');
  end if;
  if not exists (select 1 from pg_type where typname = 'payment_kind' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.payment_kind as enum ('coin_purchase', 'subscription_viewer', 'subscription_creator', 'membership', 'ppv', 'ad_settlement');
  end if;
  if not exists (select 1 from pg_type where typname = 'payment_status' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.payment_status as enum ('pending', 'succeeded', 'failed', 'refunded', 'disputed');
  end if;
  if not exists (select 1 from pg_type where typname = 'tip_kind' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.tip_kind as enum ('gift', 'extension', 'cheer', 'membership_tip');
  end if;
  if not exists (select 1 from pg_type where typname = 'stream_status' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.stream_status as enum ('scheduled', 'live', 'ended', 'cancelled');
  end if;
  if not exists (select 1 from pg_type where typname = 'stream_phase' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.stream_phase as enum ('free_30', 'extension_30', 'overgauge', 'rostime', 'grace', 'ended');
  end if;
  if not exists (select 1 from pg_type where typname = 'gauge_phase' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.gauge_phase as enum ('accumulating', 'threshold_met', 'rostime', 'grace', 'extended', 'ended');
  end if;
  if not exists (select 1 from pg_type where typname = 'revenue_event_kind' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.revenue_event_kind as enum ('gift', 'extension', 'membership', 'ppv', 'ad_share', 'infra_allocation', 'adjustment');
  end if;
  if not exists (select 1 from pg_type where typname = 'payout_status' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.payout_status as enum ('pending', 'hold', 'approved', 'processing', 'paid', 'failed', 'cancelled');
  end if;
  if not exists (select 1 from pg_type where typname = 'rank_change_reason' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.rank_change_reason as enum ('monthly_eval', 'score_ma30_promotion', 'maintain_demotion', 'legend_ppr_selection', 'legend_ppr_replacement', 'ops_override', 'trust_instant_demotion', 'channel_created');
  end if;
  if not exists (select 1 from pg_type where typname = 'stream_event_kind' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.stream_event_kind as enum ('viewer_join', 'viewer_leave', 'chat_message', 'cheer_display', 'gauge_tick', 'extension_coin', 'extension_unlock', 'overgauge_start', 'overgauge_end', 'rostime_start', 'rostime_tick', 'grace_start', 'stream_end', 'raid_offer', 'raid_execute', 'bitrate_change', 'afk_warning');
  end if;
end $$;

-- =========================================================================
-- Phase 1 ENUM additions (viewer_wallets / coin_lots / webhook)
-- =========================================================================

do $$
begin
  if not exists (select 1 from pg_type where typname = 'coin_lot_source' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.coin_lot_source as enum ('web_stripe', 'ios_iap', 'android_iap', 'welcome_grant', 'ops_adjustment');
  end if;
  if not exists (select 1 from pg_type where typname = 'payment_provider' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.payment_provider as enum ('stripe', 'apple_iap', 'google_iap');
  end if;
  if not exists (select 1 from pg_type where typname = 'provider_event_status' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.provider_event_status as enum ('received', 'processing', 'processed', 'failed', 'ignored');
  end if;
  if not exists (select 1 from pg_type where typname = 'wallet_status' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.wallet_status as enum ('active', 'frozen', 'closed');
  end if;
  if not exists (select 1 from pg_type where typname = 'wallet_ledger_entry_type' and typnamespace = (select oid from pg_namespace where nspname = 'tlv')) then
    create type tlv.wallet_ledger_entry_type as enum ('purchase_credit', 'tip_debit', 'refund_credit', 'chargeback_debit', 'adjustment_credit', 'adjustment_debit', 'lock', 'unlock');
  end if;
end $$;
...
drop trigger if exists fee_config_updated_at on tlv.fee_config;
...
drop trigger if exists creators_updated_at on tlv.creators;
...
drop trigger if exists streams_updated_at on tlv.streams;
...
drop trigger if exists payments_updated_at on tlv.payments;
...
drop trigger if exists gauge_state_updated_at on tlv.gauge_state;
...
drop trigger if exists creator_score_monthly_updated_at on tlv.creator_score_monthly;
create trigger creator_score_monthly_updated_at
  before update on tlv.creator_score_monthly
  for each row execute function tlv.set_updated_at();

-- =========================================================================
-- TABLE: creator_rank_history
-- =========================================================================
...
drop trigger if exists payout_log_updated_at on tlv.payout_log;
create trigger payout_log_updated_at
  before update on tlv.payout_log
  for each row execute function tlv.set_updated_at();

-- =========================================================================
-- TABLE: legend_waitlist
-- =========================================================================
...
-- =========================================================================
-- FK: payout_log → creator_score_monthly composite
-- =========================================================================
...
-- =========================================================================
-- Phase 1.2.3 — viewer_wallets / coin_lots / payment_provider_events
-- =========================================================================
...
drop trigger if exists viewer_wallets_updated_at on tlv.viewer_wallets;
...
drop trigger if exists coin_lots_updated_at on tlv.coin_lots;
...
-- =========================================================================
-- TABLE: tip_coin_lot_allocations
-- =========================================================================
