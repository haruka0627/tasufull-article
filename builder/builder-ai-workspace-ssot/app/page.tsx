...
import { detectIntent, QUICK_PROMPTS } from '@/lib/builder-ai'
...
function renderArtifact(kind: ArtifactKind, key: string) {
  switch (kind) {
    case 'estimate-draft':
      return <EstimateDraftCard key={key} no={NO} issueDate={DATE} title={TITLE} lineCount={7} />
    case 'cost':
      return <CostCard key={key} />
    case 'quote-preview':
      return <QuotePreviewCard key={key} no={NO} issueDate={DATE} title={TITLE} />
    case 'schedule':
...
export default function Page() {
  ...
  const send = useCallback((text: string) => {
    idRef.current += 1
    const turn: Turn = { id: idRef.current, input: text, time: now(), result: detectIntent(text) }
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background text-foreground">
      <Sidebar
        ...
        onNewChat={() => { setTurns([]); setSidebarOpen(false) }}
      />
            ...
            {turns.length === 0 ? (
              ...
              turns.map((t) => (
                <div key={t.id} className="space-y-4">
                  ...
                  {/* artifacts */}
                  {t.result.artifacts.map((a, i) => renderArtifact(a, `${t.id}-${a}-${i}`))}
                  {/* next actions inline card */}
                  <NextActions actions={t.result.nextActions} onPick={send} />
                </div>
              ))
            )}
          </div>
