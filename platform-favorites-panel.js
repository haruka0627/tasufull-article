/**
 * Platform 共通お気に入りパネル — dashboard-favorites.html でマウント
 * TasuFavoriteStore + tasful_favorite_listings（レガシー ID）を統合表示
 */
(function (global) {
  "use strict";
  ...
  function getRoot() {
    ...
    panelRoot = document.querySelector("[data-platform-favorites-root]");
    return panelRoot;
  }
  ...
  function removeSelfReloadFrames() {
    const pageRe = /(?:favorites-list|dashboard-favorites)\.html/i;
  ...
  function writeMeta(meta) {
    try {
      ...
      console.warn("[platform-favorites-panel] meta save failed:", err);
    }
  }
  ...
  function inferTypeKey(listingId) {
    ...
    if (/^mat-/i.test(id)) return "material";
    return "listing";
  }
  ...
  function matchesTypeFilter(item, filter) {
    ...
    if (filter === "business" || filter === "service" || filter === "business-service") {
      return key === "business-service" || key === "business_service" || key === "skill";
    }
    return key === filter;
  }
  ...
  function resolveFavoriteItem(id) {
    ...
    if (stored) {
      ...
      const descParts = [];
      let tags = stored.category ? [stored.category] : [];
      if (typeKey === "material") {
        if (stored.category) descParts.push(stored.category);
        if (stored.price) descParts.push(stored.price);
        tags = [stored.category, stored.price || "無料"].filter(Boolean);
        if (stored.priceType === "free") tags.push("無料素材");
      } else if (stored.price) {
        descParts.push(`価格: ${stored.price}`);
      }
      return {
        id: listingId,
  ...
  function buildTagsHtml(tags, item) {
    const chips = [...(tags || [])];
    if (item?.typeKey === "material" && item.typeLabel) {
      chips.unshift(item.typeLabel);
    }
    if (!chips.length) return "";
    return `<div class="fav-item__tags">${chips
      .map((t, i) => {
        const cls =
          t === "無料" || t === "無料素材"
            ? " fav-item__tag--free"
            : t === "有料"
              ? " fav-item__tag--paid"
              : t === "素材"
                ? " fav-item__tag--material"
                : i === 0 && item?.typeKey === "material"
                  ? " fav-item__tag--material"
                  : "";
        return `<span class="fav-item__tag${cls}">${escapeHtml(t)}</span>`;
      })
      .join("")}</div>`;
  }
  ...
  function buildItemHtml(item) {
    const titleBlock =
      item.isDemo || item.typeKey === "material"
    ...
    const detailText = item.typeKey === "material" ? "素材を見る" : "詳細を見る";

    return `
      <article class="fav-item${item.typeKey === "material" ? " fav-item--material" : ""}" data-favorite-item data-listing-id="${escapeHtml(item.id)}" data-type-key="${escapeHtml(item.typeKey)}">
        ${buildSavedBadgeHtml(item, "corner")}
        <div class="fav-item__thumb">${buildThumbHtml(item)}</div>
        <div class="fav-item__body">
          ${buildTagsHtml(item.tags, item)}
          ${titleBlock}
        ...
        <div class="fav-item__aside">
          ...
          <div class="fav-item__actions">
            <a class="fav-btn fav-btn--detail" href="${escapeHtml(wrapFavoriteDetailHref(item.detailHref))}" data-breadcrumb-label="${detailLabel}">
              ${detailText} <span class="fav-btn__chevron" aria-hidden="true">›</span>
            </a>
  ...
  function getFilteredItems() {
    ...
    if (state.typeFilter) {
      items = items.filter((item) => matchesTypeFilter(item, state.typeFilter));
    }
  ...
  function syncViewToggle() {
    ...
    getRoot()?.querySelectorAll("[data-favorites-view-btn]").forEach((btn) => {
      const active = btn.getAttribute("data-favorites-view-btn") === state.view;
      btn.classList.toggle("is-active", active);
      btn.setAttribute("aria-pressed", active ? "true" : "false");
    });
    const listEl = qs("[data-favorites-list]");
    if (listEl) {
  ...
  function syncPromo(count) {
    const statEl = qs("[data-favorites-promo-stat]");
    const subEl = qs("[data-favorites-promo-sub]");
  ...
  function renderFolderTabs(allIds) {
    const nav = qs("[data-favorites-folders]");
    const folders = window.TasuPlatformFavoriteFolders;
  ...
  function renderFavorites() {
    ...
    const listEl = qs("[data-favorites-list]");
    const panelEl = qs("[data-favorites-panel]");
    const emptyEl = qs("[data-favorites-empty]");
    const noResultsEl = qs("[data-favorites-no-results]");
  ...
  function readControlsFromDom() {
    const searchEl = qs("[data-favorites-search]");
    const typeEl = qs("[data-favorites-type-filter]");
    const sortEl = qs("[data-favorites-sort]");
  ...
  function bindControls() {
    const root = getRoot();
    if (!root || root.dataset.favoritesBound === "1") return;
    root.dataset.favoritesBound = "1";

    const searchEl = qs("[data-favorites-search]");
    const typeEl = qs("[data-favorites-type-filter]");
    const sortEl = qs("[data-favorites-sort]");
    const clearAllBtn = qs("[data-favorites-clear-all]");
    ...
    root.querySelectorAll("[data-favorites-view-btn]").forEach((btn) => {
  ...
  function start() {
    if (started) return;
    if (!getRoot() && document.body?.dataset?.page !== "dashboard-favorites") return;
    started = true;
    removeSelfReloadFrames();
    readControlsFromDom();
    bindControls();
  ...
  global.TasuPlatformFavoritesPanel = api;
  global.TasuFavoritesList = api;
  global.loadFavorites = renderFavorites;
