 ...
 * Phase B5: GET attaches sanitized result slice for dashboard read display.
 * No DeepSeek · no collector writes · no child executions.
 */
...
/**
 * Sanitized result slice for GET / dashboard (B5). No raw provider/ops payloads.
 * @param {Record<string, unknown>|null|undefined} result
 */
export function mapSanitizedResultForGet(result) {
  if (!result || typeof result !== "object") return null;
  const metrics =
    result.metrics && typeof result.metrics === "object" ? result.metrics : {};
  const pendingRaw = Number(metrics.pending_total);
  const pendingTotal =
    Number.isFinite(pendingRaw) && pendingRaw >= 0 ? pendingRaw : 0;
  const costRaw = Number(metrics.recorded_api_cost);
  const recordedApiCost =
    Number.isFinite(costRaw) && costRaw >= 0 ? costRaw : 0;
  return {
    summary: String(result.sanitized_summary || "").slice(0, 8000),
    pending_total: pendingTotal,
    provider_called: metrics.provider_called === true,
    recorded_api_cost: recordedApiCost,
    output_type: result.output_type
      ? String(result.output_type).slice(0, 128)
      : null,
    completed_at: result.completed_at || null,
    error_code: result.error_code
      ? String(result.error_code).slice(0, 128)
      : null,
  };
}
 ...
 */
export function mapGetResponse(row, events = [], result = null) {
  ...
  const sanitizedResult = mapSanitizedResultForGet(result);
  const eventsSafe = (Array.isArray(events) ? events : [])
    .slice(0, 40)
    .map((e) => ({
      sequence_number: Number(e.sequence_number) || 0,
      event_type: e.event_type ? String(e.event_type).slice(0, 64) : null,
      decision: e.decision ? String(e.decision).slice(0, 32) : null,
      blocked_reason: e.blocked_reason
        ? String(e.blocked_reason).slice(0, 64)
        : null,
  ...
  return {
    ...
    budget_day_key: row.budget_day_key || null,
    ...
    provider_called: sanitizedResult
      ? sanitizedResult.provider_called === true
      : false,
...
export async function getGateExecution(input) {
  ...
  return {
    ok: true,
    http: 200,
    body: mapGetResponse(row, events, result),
  };
}
