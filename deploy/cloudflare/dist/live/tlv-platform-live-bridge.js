...
(function (global) {
  ...
  let _adapter = null;
  /** @type {object|null} */
  let _zegoLoadResult = null;
  /** @type {Promise<object>|null} */
  let _zegoLoadPromise = null;
  ...
  /** Phase5 P5-5 · ZEGO provider（Flag ON 時のみ lazy · PLATFORM_LIVE_SCRIPTS とは分離） */
  const ZEGO_LIVE_SCRIPTS = Object.freeze([
    "providers/zego-live-provider.js",
    "../platform-live/provider/adapters/zego-live-provider-adapter.js",
  ]);

  /** @returns {boolean} */
  function isZegoProviderReady() {
    return Boolean(global.TlvZegoLiveProvider && global.ZegoLiveProviderAdapter);
  }

  /** @returns {string} */
  function resolveAdapterProviderId() {
    return isZegoProviderReady() ? "zego" : "stub";
  }
  ...
  /**
   * Phase5 P5-5 · ZEGO provider lazy load（Flag OFF 時 no-op · 失敗 non-fatal）
   * @returns {Promise<{ ok: boolean, ready?: boolean, skipped?: boolean, partial?: boolean, error?: string, reason?: string }>}
   */
  async function ensureZegoProviderLoaded() {
    if (!isEnabled()) {
      const res = { ok: true, skipped: true, reason: "usePlatformLive_disabled", ready: false };
      _zegoLoadResult = { ...res, at: new Date().toISOString() };
      return res;
    }
    if (isZegoProviderReady()) {
      _zegoLoadResult = { ok: true, ready: true, at: new Date().toISOString() };
      return _zegoLoadResult;
    }
    if (_zegoLoadPromise) return _zegoLoadPromise;

    _zegoLoadPromise = (async () => {
      const errors = [];
      for (const src of ZEGO_LIVE_SCRIPTS) {
        try {
          await _loadScript(src);
        } catch (err) {
          errors.push(`${src}: ${err?.message || String(err)}`);
        }
      }
      const ready = isZegoProviderReady();
      const result = {
        ok: true,
        ready,
        partial: !ready,
        error: ready ? undefined : errors.join(" | ") || "ZEGO provider globals missing",
        at: new Date().toISOString(),
      };
      if (!ready) {
        console.warn("[TlvPlatformLiveBridge] ZEGO provider lazy load:", result.error);
      }
      _zegoLoadResult = result;
      return result;
    })().catch((err) => {
      _zegoLoadPromise = null;
      const result = {
        ok: false,
        ready: false,
        partial: true,
  ...
  async function _getAdapter() {
    ...
    if (!_adapter) {
      await ensureZegoProviderLoaded();
      const providerId = resolveAdapterProviderId();
  ...
  function getDiagnostics() {
    ...
    const providerId = adapterDiag?.adapter?.providerId || _adapter?._providerId || resolveAdapterProviderId();
    return {
      enabled: isEnabled(),
      integrationLoaded: Boolean(global.TasuLivePlatformIntegration),
      adapterReady: Boolean(_adapter),
      zegoProviderReady: isZegoProviderReady(),
      zegoProviderLoad: _zegoLoadResult ? { ..._zegoLoadResult } : null,
      providerId,
