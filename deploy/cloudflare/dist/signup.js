...
(function initSignupPage() {
  ...
  document.querySelectorAll("[data-signup-social]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (provider === "line") {
        showToast("LINE 登録は現在利用できません。Google またはメール登録をご利用ください。", "info");
        return;
      }
      if (provider === "google") {
        const r = await window.TasuPlatformGoogleAuth?.signInWithGoogle?.({ returnTo: "dashboard.html" });
        if (!r?.ok) showToast(r?.message || "Google 登録を開始できませんでした。", "error");
        return;
      }
      showToast("ソーシャル登録は準備中です。", "info");
    });
  });
