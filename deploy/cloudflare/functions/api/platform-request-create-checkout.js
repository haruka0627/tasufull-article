/**
 * Platform Request P5-10 — Stripe Checkout create (Staging Test · service_role Edge)
 *
 * POST /api/platform-request-create-checkout
 * Authorization: Bearer <Supabase access_token>
 * Body: { request_id, match_id, success_url?, cancel_url?, origin? }
 */

import {
  assertStagingUrl,
  createPlatformRequestCheckout,
  getSupabaseConfig,
  jsonResponse,
  pickStr,
  verifySupabaseJwt,
...
export async function onRequest(context) {
  ...
  var result = await createPlatformRequestCheckout(cfg, env, userId, body, request);
  if (!result.ok) {
    return jsonResponse(
