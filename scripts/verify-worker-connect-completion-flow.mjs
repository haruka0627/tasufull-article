#!/usr/bin/env node
/**
 * worker Connectあり — B承認後に completed + 取引完了カード + レビュー通知
 */
import { launchHeadlessBrowser } from "./lib/playwright-browser.mjs";
import { requireDevServer } from "./lib/dev-base-url.mjs";

const BASE = await requireDevServer();
const THREAD_ID = "chat-demo-worker-deal-001";
const SELLER_ID = "demo-worker-001";
const BUYER_ID = "u_hiro";
const LISTING_ID = "demo-worker-001";
...
try {
  ...
  await page.evaluate(
    ({ threadId, sellerId, buyerId, listingId }) => {
      ...
      threads.unshift({
        ...
        completionDeliverySummary: "作業完了しました",
        source: "chat-dual-window-demo",
        lastMessage: "作業完了を申請しました",
        createdAt: now,
        updatedAt: now,
      });
  ...
  const preApprove = await page.evaluate((threadId) => {
    ...
    return {
      ...
      manual: window.TasuPlatformChatCompletionFlow?.requiresManualTransferAfterApproval?.(thread),
    };
  }, THREAD_ID);
  if (preApprove.isWorkService !== false) {
    errors.push(`worker connect misclassified as work service ${JSON.stringify(preApprove)}`);
  }
  ...
  if ((await approveBtn.count()) < 1) {
    ...
    if (!/作業内容を確認/.test(btnLabel)) {
      errors.push(`B button label expected 作業内容を確認 got: ${btnLabel}`);
    }
...
console.log("OK worker connect completion flow");
