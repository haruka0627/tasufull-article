#!/usr/bin/env node
/**
 * ANPI Phase 48 — Scheduled runtime accelerated fixture tests (local Supabase)
 *
 * Scenarios A–I using Phase 48 entrypoint (reuses Phase 47 core).
 * Timing is forced via fixture scheduled_at / available_at — never mutates prod settings.
 */
...
async function main() {
  ...
  add("A: scheduled → notified (initial)", async () => {
  ...
  add("B: first reminder → reminded", async () => {
  ...
  add("E: contact_unconfirmed → contact_notified", async () => {
  ...
  add("I: concurrent runners — one acquires, no duplicate delivery", async () => {
    ...
    try {
      ...
      // One PASS and one SKIPPED(busy), or both PASS if lease window allowed sequential — still no dup.
