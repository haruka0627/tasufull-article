#!/usr/bin/env node
/**
 * TLV watch drawer open/close verify (localhost :8788)
 * Usage: node scripts/verify-tlv-watch-drawer.mjs
 */
    ...
    async ({ browser }) => {
      for (const vp of [
        ...
        const openState = await page.evaluate(() => {
          ...
          return {
            ...
            drawerVisible: drawer
              ? (dcs.display !== "none" && dcs.transform === "none") ||
        ...
        const afterBackdrop = await page.evaluate(() =>
          document.body.classList.contains("tlv-drawer-open"),
        );
        ...
        const closedByBackdrop = !afterBackdrop;
        const closedByEsc = !afterEsc;
        const layoutOk =
          before?.layoutW != null &&
          before?.player != null &&
          after?.layoutW != null &&
        ...
        if (!openState.hasDrawer) failures.push(`viewport ${vp.w}: drawer element missing`);
        if (!openState.hasBackdrop) failures.push(`viewport ${vp.w}: backdrop element missing`);
        if (!openState.bodyOpen && !openState.drawerVisible) {
          failures.push(`viewport ${vp.w}: drawer did not open`);
        }
        if (!closedByBackdrop) failures.push(`viewport ${vp.w}: closedByBackdrop=false`);
        if (!closedByEsc) failures.push(`viewport ${vp.w}: closedByEsc=false`);
        if (!layoutOk) failures.push(`viewport ${vp.w}: layout metrics incomplete`);
