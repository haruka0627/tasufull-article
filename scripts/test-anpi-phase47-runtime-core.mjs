#!/usr/bin/env node
/**
 * ANPI Phase 47 — Notification Runtime Core unit tests (local Supabase)
 *
 ...
 * - contact notification for eligible contacts (overdue → contact_notified)
 * - unverified / inactive contacts excluded from contact notifications
 * - confirmed_late → late_confirmation candidate generation (candidate only)
...
async function main() {
  ...
  add("当日生成 + 同日重複防止 + initial delivery", async () => {
  ...
  add("confirmed stop (本人confirm→以降通知なし)", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
    ...
    try {
      ...
      // Simulate "本人 confirmed" by setting status to confirmed with required fields.
  ...
  add("paused stop (pausedチェックは通知されない)", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
  ...
  add("cancelled stop (cancelledチェックは通知されない)", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
  ...
  add("contact通知（overdue→contact_notified＋contact delivery）", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
    const users = {
  ...
  add("未承認連絡先除外（verification未達→contact通知なし）", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
    const users = {
  ...
  add("inactive連絡先除外（status除外→contact通知なし）", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
    const users = {
  ...
  add("confirmed_late → 完了通知候補（late_confirmation）/ 重複防止 + no delivery in phase47", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
    const users = {
  ...
  add("timezone / 日付境界（local_check_dateがTokyoで正しい）", async () => {
    const apiUrl = env.apiUrl;
    const serviceKey = env.serviceKey;
