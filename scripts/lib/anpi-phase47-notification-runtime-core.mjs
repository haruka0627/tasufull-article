/**
 * ANPI Phase 47 — Notification Runtime Core (staging-safe simulation)
 *
 * Goals (this phase):
 * - Never enable Production runtime, never perform external sends.
 * - Use existing Phase 2–10 schema/state-machine via RPCs where available.
...
export async function runAnpiPhase47NotificationRuntimeCore({
  ...
  // 1) due check + scheduled→notified + overdue + contact candidates (DB-side)
  ...
  // 2) reminder cadence candidate generation — runtime side
