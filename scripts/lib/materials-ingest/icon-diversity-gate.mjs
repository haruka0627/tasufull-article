/**
 * Icon exact-duplicate + spec-diversity gates.
 * Reuses existing-register sha256. No perceptual SVG/image similarity engine.
 */
import crypto from "node:crypto";
import { buildDuplicateIndex, checkDuplicate, sha256File } from "./existing-register/hash.mjs";
import { iconSpecFingerprint } from "./icon-demand-genre-ssot.mjs";
...
export const ICON_NEAR_DUPLICATE_GATE_STATUS = "REVIEW_REQUIRED";
export const ICON_NEAR_DUPLICATE_GATE_REASON =
  "NOT_AVAILABLE — repository has no near-icon SVG/image similarity; hash≠near-duplicate";

export function sha256Buffer(buf) {
  return crypto.createHash("sha256").update(buf).digest("hex");
}

export function createIconDiversityState(existingItems = []) {
  ...
  for (const item of existingItems) {
    ...
    const fp = item?.spec_fingerprint || iconSpecFingerprint(item);
...
export function checkIconGenerationGates(candidate, state) {
  const reasons = [...appendCanonicalGenreGateReasons(candidate, "icon")];
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || iconSpecFingerprint(candidate.spec || candidate);
  ...
  return {
    ok: reasons.length === 0,
    reasons,
    near_icon: { status: ICON_NEAR_DUPLICATE_GATE_STATUS, reason: ICON_NEAR_DUPLICATE_GATE_REASON },
  };
}

export function rememberIconCandidate(state, candidate, sha256) {
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || iconSpecFingerprint(candidate.spec || candidate);
