/**
 * Image exact-duplicate + spec-diversity gates.
...
import { imageSpecFingerprint } from "./image-demand-genre-ssot.mjs";
import { appendCanonicalGenreGateReasons } from "./canonical-genre-generation-contract-v1.mjs";

export const IMAGE_NEAR_DUPLICATE_GATE_STATUS = "REVIEW_REQUIRED";
export const IMAGE_NEAR_DUPLICATE_GATE_REASON =
  "Perceptual image similarity is HUMAN_VISUAL; spec fingerprint is fail-closed. Hash-only uniqueness is forbidden.";

export function sha256Buffer(buf) {
  return crypto.createHash("sha256").update(buf).digest("hex");
}

export function createImageDiversityState(existingItems = []) {
  ...
  for (const item of existingItems) {
    ...
    const fp = item?.spec_fingerprint || imageSpecFingerprint(item);
...
export function checkImageGenerationGates(candidate, state) {
  const reasons = [...appendCanonicalGenreGateReasons(candidate, "image")];
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || imageSpecFingerprint(candidate.spec || candidate);
  ...
  return {
    ...
    reasons,
    near_image: {
      status: IMAGE_NEAR_DUPLICATE_GATE_STATUS,
      reason: IMAGE_NEAR_DUPLICATE_GATE_REASON,
    },
  };
}

export function rememberImageCandidate(state, candidate, sha256) {
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || imageSpecFingerprint(candidate.spec || candidate);
