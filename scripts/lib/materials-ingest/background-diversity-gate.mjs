/**
 * Background exact-duplicate + spec-diversity gates.
...
import { backgroundSpecFingerprint } from "./background-demand-genre-ssot.mjs";
import { appendCanonicalGenreGateReasons } from "./canonical-genre-generation-contract-v1.mjs";

export const BACKGROUND_NEAR_DUPLICATE_GATE_STATUS = "REVIEW_REQUIRED";
export const BACKGROUND_NEAR_DUPLICATE_GATE_REASON =
...
export function createBackgroundDiversityState(existingItems = []) {
  ...
  for (const item of existingItems) {
    ...
    const fp = item?.spec_fingerprint || backgroundSpecFingerprint(item);
...
export function checkBackgroundGenerationGates(candidate, state) {
  const reasons = [...appendCanonicalGenreGateReasons(candidate, "background")];
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || backgroundSpecFingerprint(candidate.spec || candidate);
  ...
  return {
    ok: reasons.length === 0,
    reasons,
    near_image: { status: BACKGROUND_NEAR_DUPLICATE_GATE_STATUS, reason: BACKGROUND_NEAR_DUPLICATE_GATE_REASON },
  };
}

export function rememberBackgroundCandidate(state, candidate, sha256) {
  const title = String(candidate.title || "").trim();
  const prompt = String(candidate.prompt || candidate.promptText || "").trim();
  const fp = candidate.spec_fingerprint || backgroundSpecFingerprint(candidate.spec || candidate);
