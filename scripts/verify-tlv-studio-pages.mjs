#!/usr/bin/env node
/**
 * TLV Studio pages nav verify (localhost :8788)
 * Usage: node scripts/verify-tlv-studio-pages.mjs
 */
    ...
    async ({ browser }) => {
      ...
      for (const vp of [
        ...
        for (const item of PAGES) {
          ...
          const state = await page.evaluate((navId) => {
            ...
            return {
              ...
              httpOk: true,
              activeId:
                active?.querySelector(".tlv-studio-sidebar__link-label")?.textContent?.trim() ||
                null,
              ...
              mobileShell:
                getComputedStyle(document.querySelector(".tlv-studio-mobile-shell")).display !==
                "none",
              desktopShell:
                getComputedStyle(document.querySelector(".tlv-studio-shell")).display !== "none",
            };
          }, item.navId);
        ...
        for (const r of results) {
          if (r.activeOk === false) {
            failures.push(
              `viewport ${vp.w}: ${r.page} activeOk=false (activeId=${JSON.stringify(r.activeId)})`,
            );
          }
        }
