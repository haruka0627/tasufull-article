#!/usr/bin/env node
/**
 * Phase 2-D: fix detail-business-service.html encoding (DOM / scripts / data-* preserved).
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = path.resolve(import.meta.dirname, "..");
const TARGET = "detail-business-service.html";
...
function fixBusinessServiceHtml(text) {
  let out = text;
...
const after = fixBusinessServiceHtml(before);
fs.writeFileSync(abs, after, "utf8");

const ufffd = (after.match(/\uFFFD/g) || []).length;
const brokenClose =
  (after.match(/[^<]\/(?:span|a|h[1-6]|div|p|section|button|th)>/g) || []).length;
