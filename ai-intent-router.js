...
(function (global) {
  ...
  const NON_CATALOG_PATTERNS = [
    ...
    /コードから|コードの|このコード|ソースコード|リファクタ|関数を|実装を|求人コード|コードを直|業者コード|業務サービス.*コード/,
    /探し方を|探し方は|探し方について|おすすめの探し方|どうやって探|検索の(仕方|方法|使い方)/,
    /検索機能について|検索について説明|サイトの検索|業者の探し方|業務サービスの探し方/,
    /要約して|翻訳して|コードを書いて|アイデアを出して|この画像を説明|説明を書いて|サービス説明を書/,
    /抽出して|条件を抽出|募集条件を抽出|この文章から/,
  ];
  ...
  function classifyIntent(userText) {
    ...
    if (
      ...
      /いる|探|頼|依頼/.test(text) &&
      // Phase 4: スキル領域のみ（ワーカー固有語なし）は worker に落とさない
      !(
        /スキル|デザイン|動画編集|ライティング|プログラミング|コーディング|イラスト|翻訳|マーケ/i.test(
          text
        ) && !/ワーカー|作業員|人手|力仕事|軽作業|搬入|荷物|明日|今日/i.test(text)
      )
    ) {
    ...
    if (
      ...
      /いる|探|ある|相談|頼|手伝/.test(text) &&
      !/コード|バグ|エラー|デバッグ/.test(text) &&
      // Phase 4: ワーカー固有語が主の依頼は skill に落とさない
      !/ワーカー|作業員|人手|力仕事|軽作業|搬入|荷物/i.test(text)
    ) {
