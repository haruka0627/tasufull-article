...
| **6-C Gmail read-only** | messages.list/get · threads.get · labels.list · q · 添付 metadata · UI カード | ✅ 実装 — `reports/secretary-google-phase6c-gmail-readonly.md` |
| **6-D〜H** | Gmail write · Calendar · Contacts · Drive（段階実装） | 📋 未着手 |
...
**禁止（6-D まで）:** `messages.send` · `drafts.*` · `messages.trash` · `messages.delete` → `gmail_write_forbidden`
