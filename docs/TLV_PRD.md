...
**運営方針正本:** 本書 **§0 · §1.1**（ライブ配信体験 · ショート動画体験 · AI/TALK 境界）。経済・Score の根幹は AD-014 と両立。  
...
| ⑩ | **プロダクト範囲** = **ショート動画 + ライブ配信** 特化 · 総合動画サービスは目指さない（§0） |
...
| [LIVE_PLATFORM_CONCEPT.md](./LIVE_PLATFORM_CONCEPT.md)「通常動画 → ショート → ライブ」 | **経済・流入の概念図は維持**。**TLV プロダクトの運営焦点**は **ショート + ライブ**（通常ロング VOD は TLV コア対象外 · §0.1） |

---

## 0. 運営方針（Product Scope · 2026-07-26）

本節は **機能追加の方向性を固定**する運営正本である。Score / 還元 / 30 分サバイバル経済（§3–§7）は変更しない。v1.0 FROZEN コードへの実装を強制しない（設計・次世代優先順位の指針）。

### 0.1 基本コンセプト

**TLV は「ショート動画」と「ライブ配信」に特化した配信プラットフォームである。**

| する | しない |
| --- | --- |
| リアルタイム配信体験の品質向上 | 総合動画サービス（長尺ライブラリ中心の YouTube 型） |
| ショートによる集客 · 視聴 · **ライブへの導線** | 汎用 SNS · 動画編集サービス |
| 見つけやすさ · 応援しやすさ · 軽快な動作 | 配信と無関係な多機能の積み上げ |

**目指す方向性（一文）**

> **配信者は配信しやすく、視聴者は見つけやすく、応援しやすいサービス**

継続優先: **配信品質 · 見つけやすさ · 応援しやすさ · 安定性 · 軽快な動作**

### 0.2 ライブ配信（対応予定・優先スコープ）

| 機能 | 位置づけ | 備考 |
| --- | --- | --- |
| 個人ライブ | コア | 既存 v1 / Provider 系と接続 |
| コラボライブ | 優先 | 相談 UX は TALK 集約（§0.5） |
| イベント配信 | 優先 | [EVENT_SYSTEM.md](./EVENT_SYSTEM.md) と整合 |
| ギフト · 応援 | コア | 経済正本は本書 · Payment Engine |
| ランキング | コア | ルーム内 / 制度 Rank は §5–§6 · **AI による操作なし** |
| 配信通知 | 優先 | Push / アプリ通知は別設計 |
| アーカイブ | 必要に応じて | 必須コアではない · infra コスト意識 |
| 配信スケジュール | 優先 | AI は予約**支援**まで（§0.4） |
| フォロー中配信一覧 | 優先 | 見つけやすさ |

**コア経済（既存・不変）:** 30 分無料 → 条件達成時のみ延長 · コイン · Score · Rank（§4–§6）

### 0.3 ショート動画（ライブへの導線）

ショート動画は **目的そのものではなく、ライブ配信への導線** として位置づける。

| 対応予定 | 役割 |
| --- | --- |
| 縦動画 | モバイル第一の視聴体験 |
| おすすめ表示 | 発見 · ただし **露出保証・ランキング操作はしない**（§0.4） |
| フォロー · いいね · コメント · シェア | 参加 · 拡散 |
| ライブ配信への誘導 | **最優先 UX** |
| 配信予定の告知 | スケジュール · 通知と連動 |

**非目標:** ショート単体での総合 SNS 化 · 編集スイート化 · 長尺ライブラリの代替。

### 0.4 TASFUL AI 連携（AD-004 維持）

TLV **専用 AI エンジンは作らない**（[DECISIONS.md](./DECISIONS.md) AD-004 · [AI/TLV_AI.md](./AI/TLV_AI.md)）。入口は `source=tlv` → TASFUL AI Workspace。

| AI が支援してよい | AI が行ってはならない |
| --- | --- |
| 配信検索 · 配信者検索 · ショート動画検索 | ランキングの操作 · 順位の改ざん |
| おすすめ表示（提案・要約ベース） | 露出の保証 · 有料枠の代替としての不当優遇 |
| 配信予約の下書き · 提案 | 配信開始・課金・還元の自動確定（AD-006 ドラフト原則と整合） |

AI の上限: **検索 · 要約 · 提案まで**。

### 0.5 TASFUL TALK 連携（チャット非重複）

TLV 内に **独自チャットを重複実装しない**。次は **TALK へ集約**する。

| TALK へ集約 | TLV 側に残すもの |
| --- | --- |
| 配信者とのやり取り | ライブ中のルームコメント / スタンプ等（視聴体験コア） |
| コラボ相談 · 案件相談 | 配信・ショートの公開 UI |
| 運営通知（ユーザー向け） | システムトースト · 配信状態表示 |
| ファンとのコミュニケーション（1:1 / グループ） | フォロー · いいね等の軽量ソーシャル |

### 0.6 運営原則（優先順位）

1. **ライブ配信体験**と**ショート動画体験**の品質向上を最優先する  
2. 多機能化そのものを目的にしない — **配信に直接関係する機能**を優先する  
3. 経済・不正・infra（§3 · §7 · §8）は既存正本に従う — 体験拡充で Profit First を壊さない  
4. Platform Vision（AD-014）のミッション「頑張ったクリエイターが一番報われる」は維持する  
...
**TLV（TASFUL Live Video）** は、**ショート動画**と**ライブ配信**に特化した配信プラットフォームである。体験のコア経済は **30 分サバイバル配信**（§4）とする。

- **プロダクト範囲:** ショート + ライブ（§0）— 総合動画サービスは目指さない
...
- AI は TASFUL AI 入口のみ（AD-004）· チャット重複は TALK 集約（§0.5）
...
| 軸 | TLV | 既存大手 |
| --- | --- | --- |
| プロダクト範囲 | **ショート + ライブ特化**（総合 VOD 非目標 · §0） | 長尺ライブラリ + ライブ併存が多い |
...
| チャット | **TALK 集約**（TLV 内重複チャット禁止 · §0.5） | アプリ内 DM 併設が多い |
...
| 10 | プロダクト = **ショート + ライブ特化** · 総合動画・汎用 SNS・編集スイートは非目標（§0） |
| 11 | ショート = **ライブへの導線** · AI = 検索/要約/提案のみ · チャット重複は **TALK 集約** |
...
| ドキュメント | 内容 |
| --- | --- |
| 本書 §0 · §1 | **TLV 運営方針 · プロダクト範囲**（ショート + ライブ特化） |
| [LIVE_PLATFORM_CONCEPT.md](./LIVE_PLATFORM_CONCEPT.md) | AD-014 ミッション · コアバリュー · 流入概念 |
| [LIVE_SYSTEM.md](./LIVE_SYSTEM.md) | ライブ制度（30 分 · ゲージ等） |
| [AI/TLV_AI.md](./AI/TLV_AI.md) | TASFUL AI 導線 · 支援範囲（検索/要約/提案） |
...
| [DECISIONS.md](./DECISIONS.md) | AD-004 · AD-014 |
...
| 日付 | 版 | 内容 |
| --- | --- | --- |
| 2026-07-26 | **1.3 PRODUCT-SCOPE** | §0 運営方針追加 · §1.1/1.3/10.2 をショート+ライブ特化 · AI/TALK 境界に同期（Score/還元/30分経済は不変 · 実装変更なし） |
