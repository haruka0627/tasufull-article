...
**ステータス:** Ver2 **Demo Payment Flow Complete** — CSV・payment-history まで実装済み。Transfer API は未実装。

**本ドキュメントは送金候補データの設計書です。実送金は行いません。Stripe API は呼び出しません。**

**本番移行:** [TODO_FOR_PRODUCTION.md](TODO_FOR_PRODUCTION.md)
...
| `creator_name` | 表示名 |
| `stripe_connect_account_id` | Connect アカウント ID（デモ: `acct_demo_*` from `live/data/tlv-stripe-connect-accounts.json`） |
